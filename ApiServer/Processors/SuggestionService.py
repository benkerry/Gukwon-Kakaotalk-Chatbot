...
def process(db_manager, logger, dict_json:dict) -> dict:
    str_userval = None
    str_utterance = None
    ...
    cursor = db_manager.mysql_query("SELECT * FROM authed_user WHERE user_val='{0}'".format(str_userval))
    ...
    if cursor.rowcount == 0:
        ...
        if ('(' in str_utterance and ')' in str_utterance) and (str_utterance.find('(') < str_utterance.find(')')):
            ...
            str_description = html.escape(str_description)
            str_sql = "INSERT INTO suggestion(user_val, description, status, num_signs, open_datetime) VALUES('{0}', '{1}', {2}, {3}, '{4}')".format(str_userval, str_description, 0, 0, datetime.now().strftime("%Y-%m-%d"))

            db_manager.mysql_query(str_sql)
            
            str_sql = "SELECT idx FROM suggestion WHERE user_val='{0}' AND description = '{1}' AND status = 0".format(str_userval, str_description)
            issue_number = db_manager.mysql_query(str_sql).fetchone()[0]
            ...
            if numeric_idx_end == 0:
                ...
                if str_sug_idx in lst_signed:
                    ...
                    lst_sql.append("UPDATE authed_user SET signed_suggestion='{0}' WHERE user_val = '{1}'".format(str_signed + ":{0}".format(str_sug_idx), str_userval))
